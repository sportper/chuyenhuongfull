const API_URL = process.env.linkapi
const numposthome = process.env.setup.numposthome
...
export async function getposthome(page =1) {
  ...
  const data = await fetchAPI(
    ...
    query AllPosts {
  posts(
    where: {orderby: {field: DATE, order: DESC}, offsetPagination: {offset: ${pageofset}, size: ${numposthome}}}
  ) {
    edges {
      node {
        title
...
export async function getPostDetailsByUri(slug) {
  const data = await fetchAPI(`query AllPosts($slug: String = "${slug}") {
  postBy(slug: $slug) {
    author {
      node {
        lastName
